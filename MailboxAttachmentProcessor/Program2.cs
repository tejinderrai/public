...
using MailBoxAttachmentProcessor;
...
namespace CourseCompletionsProcessor
{
    internal class Program
    {
        static async Task Main()
        {
            ...
            References.LocalAttachmentDowloadPath = configuration["AppSettings:LocalStoragePath"] ?? "";

            // Retrieve the messages from the specified mailbox folder - with app credentials
            //var messages = await MSGraphOperations.GetMessageCollection();

            // Retrieve the messages from the specified mailbox folder - with Managed Identity
            var messages = await MSGraphOperations.GetMessageCollectionWithManagedIdentity();
            ...
            { 
                foreach (var message in messages.Value)
                {
                    ...
                    //{
                        ...
                        var attachments = await MSGraphOperations.GetMessageAttachmentCollectionWithManagedIdentity(message);
                    ...
                    foreach (var attachment in attachments.Value)
                    {
                        ...
                        if (attachment is FileAttachment fileAttachment)
                        {
                            ...
                            bool uploadResult = await LocalStorageOperations.DownloadAttachmentToLocalFolder(item, References.LocalAttachmentDowloadPath);
